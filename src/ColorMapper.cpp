...
PixelRGB ColorMapper::hsvToRGB(const HSV& hsv) {
    float c = hsv.v * hsv.s;
    float x = c * (1 - std::fabs(std::fmod(hsv.h / 60.f, 2.f) - 1.f));
    float m = hsv.v - c;
    ...
    PixelRGB out;
    out.r = static_cast<uint8_t>(std::round((r + m) * 255.f));
    out.g = static_cast<uint8_t>(std::round((g + m) * 255.f));
    out.b = static_cast<uint8_t>(std::round((b + m) * 255.f));
    return out;
}